...
typedef struct
{
    STATE_stateOuterTransitionTable_t const  *OuterTransitionTable;
    uint16_t const *OuterTransitionTableSize;
    STATE_stateActionTable_t const *actionTable; 
    uint16_t const *actionTableSize;
    STATE_state_t state;
} FSM_t;
...
/**
 * This function will initialise the Child state machine and process any state actions of the initial state
 * The state machine is implemented as a generic object, Several instances and configurations are supported
 * \param STATE_stateActionTable_t const * const actionTable            : IN - Pointer to the action table
 * \param uint16_t actionTableSize                                      : IN - action table size 
 * \param STATE_state_t state                                           : IN/OUT - Leaving and Resulting state 
 * \param STATE_state_t Init_state                                      : IN - Initialisation State
 * \return RC_SUCCESS or error
 */
RC_t ChildSTATE_initState(   STATE_stateActionTable_t const actionTable, 
                        uint16_t actionTableSize,
                        STATE_state_t* state, STATE_state_t Init_state);
