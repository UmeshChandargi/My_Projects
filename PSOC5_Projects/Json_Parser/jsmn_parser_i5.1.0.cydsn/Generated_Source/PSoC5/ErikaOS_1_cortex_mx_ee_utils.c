...
#if (Number_of_ISRs > 0) && defined isr_uart1__INTC_NUMBER
    ramVectorTable[CY_INT_IRQ_BASE + (isr_uart1__INTC_NUMBER & CY_INT_NUMBER_MASK)] = isr_uart1;
	//PF v2.5.3
    //NVIC_SET_PRI(isr_uart1__INTC_NUMBER, (ISR_MIN_PRI - 4));
    CyIntSetPriority(isr_uart1__INTC_NUMBER, ISR_SCALE(4));
    //\PF
	NVIC_INT_ENABLE(isr_uart1__INTC_NUMBER);
#elif Number_of_ISRs > 0
#error  The Interrupt isr_uart1 could not be found. Make sure the name in the OS config exactly matches the name of the interrupt in the interrupt tab.
#endif	/* isr_uart1 */
