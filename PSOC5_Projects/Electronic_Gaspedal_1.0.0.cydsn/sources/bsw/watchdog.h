...
*
* Driver for reading the Watchdog
*
* \note <notes>
...
/*****************************************************************************/
/* The watchdog counts each time
the Central Time Wheel (CTW) reaches the period specified. The watchdog must
be cleared using the CyWdtClear() function before the watchdog counts to three.
Reset will occur after between 2 and 3 timer periods.
The central timewheel is a 1-kHz, free-running, 13-bit counter clocked by the ILO. 
Each tick takes 1ms. For Eg- 2 ticks it should take 2ms and reset will occur between
2 and 3 timer periods i.e. 4ms - 6ms
 */
    typedef enum{ 
    Ticks_2= 0u, 
    Ticks_16= 1u, 
