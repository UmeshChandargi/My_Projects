 ...
 *
 * description: To calculate engine speed from the Joystick commands
 * name: swc_control
 * shortname: control
 *
 */
 ...
 * cycletime: 0
 * description: Runnable to calculate engine speed
...
void CONTROL_calcControl_run(RTE_event ev){
    ...
    SC_JOYSTICK_data_t joystick_value;
    
    joystick_value= RTE_SC_JOYSTICK_get(&SO_JOYSTICK_signal);
    if(joystick_value.y<=0)
    {
        speed.value=0;
        RTE_SC_SPEED_set(&SO_ENGINE_SPEED_signal,speed);
    }
    else if(joystick_value.y>0)
    {
        
        speed.value=joystick_value.y*2;
        RTE_SC_SPEED_set(&SO_ENGINE_SPEED_signal,speed);
    }
